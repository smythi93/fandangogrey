...
def run_fandango(fan: Path, put: Path, args: str, experiment_output_file: Path, timelimit: int, population_size: int):
    command = [
        "fandango",
        "fuzz",
        "--no-cache",
...
def main(config: dict, only_generate_inputs: bool):
    ...
    for n_spec, spec in enumerate(config.get("specifications", [])):
        ...
        ax.tick_params(axis='both', labelsize=12) # increase x,y ticks
    ...
    ax.set_ylabel(config.get("y-label", "Score"), fontsize=16)
    #ax.set_title(config.get("title", "no title"))
